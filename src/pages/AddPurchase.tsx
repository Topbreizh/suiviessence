...
export default function AddPurchase() {
  ...
  const { vehicles, fetchVehicles, addFuelPurchase, fetchGasStations, stores, fetchStores } = useStore();
  ...
  useEffect(() => {
    ...
    fetchGasStations();
    fetchStores();
  }, [fetchVehicles, fetchGasStations, fetchStores]);
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Votre achat de carburant a été enregistré avec succès.",
      });
  ...
  // Get selected vehicle for fuel type
  const vehicleId = form.watch("vehicleId");
  const selectedVehicle = vehicles.find((v) => v.id === vehicleId);

  // Set fuel type when vehicle changes
  useEffect(() => {
    if (selectedVehicle) {
      form.setValue("fuelType", selectedVehicle.fuelType);
    }
  ...
  return (
    <div className="container mx-auto py-6">
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          <CardTitle className="text-2xl">Ajouter un achat de carburant</CardTitle>
          <CardDescription>
            Enregistrez votre dernier achat de carburant pour suivre vos dépenses.
          </CardDescription>
        </CardHeader>
        <Form {...form}>
